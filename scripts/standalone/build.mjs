...
/** Read a file from /public (or an absolute path) as a data URI, once. */
...
/** The five webfonts, inlined into an @font-face block. */
function fontFaces() {
...
/* ── Content prepared for the client ────────────────────────────────── */
...
console.log(`Gesamt: ${(total / 1024 / 1024).toFixed(1)} MB, größte Seite: ${(Math.max(...written.map((f) => readFileSync(join(OUT, f)).length)) / 1024 / 1024).toFixed(1)} MB`)
