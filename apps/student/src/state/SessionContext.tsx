...
export function SessionProvider({ children }: { children: ReactNode }) {
  ...
  const login = useCallback(
    async (email: string, password: string, totpToken?: string) => {
      const body = await apiMutate<{ user: SessionUser }>("/auth/login", "POST", {
        email,
        password,
        totpToken,
      });
  ...
  const logout = useCallback(async () => {
    try {
      ...
      if (!(err instanceof ApiError)) {
        ...
        // aber keine falsche "erfolgreich abgemeldet" Behauptung nötig -
        // die UI zeigt in diesem Fall einfach den Login-Screen wieder.
      }
    }
    setUser(null);
  }, []);
